...
function PatientAppointmentTable(props) {
    ...
    useEffect(() => {
        if (!isEmpty(props.doctor)){
            setAppointmentData(generateAppointmentList(props.appointments, props.doctor));
        }
    ...
    const removeAppointment = (id) => {
        ...
        props.addNotification({
            ...
            message: `[${getFormattedDate(new Date())}] Your patient ${props.auth.userData["first_name"]} ${props.auth.userData["last_name"]} has cancelled an appointment.`
        });
    }
    ...
    if(props.appointments.length == 0){
        ...
        return (
            <Fragment>
                ...
                <TableContainer component={Paper}  sx={{ width: "80%", margin: 'auto', my: 6}}>
                    ...
                    <TableBody>
                        {appointmentData.map((item, i) => (
                            ...
                            >
                                ...
                                <TableCell>
                                    <Button size="small" variant="contained" color="error" onClick={() => removeAppointment(item["id"])}>
                                        Cancel
                                    </Button>
                                </TableCell>
                            </TableRow>
...
export default connect(mapStateToProps, { getDoctor, getAppointments, deleteAppointment, createMessage, addNotification })(PatientAppointmentTable);
